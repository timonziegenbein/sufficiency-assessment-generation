...
def average_state_dicts(state_dicts: List[Dict[str, torch.Tensor]]):
    ...
    for k in state_dicts[0].keys():
        ...
        new_t = torch.true_divide(sum(tensors),len(tensors))
...
if __name__ == "__main__":
    files_to_convert = [
        'val_BERTScoreF1=32.029.ckpt',
        'val_BERTScoreF1=31.873.ckpt',
        'val_BERTScoreF1=34.055.ckpt',
        'val_BERTScoreF1=32.266.ckpt',
        'val_BERTScoreF1=30.635.ckpt'
    ]
    for i, file in enumerate(files_to_convert):
        if i >=7:
            convert_pl_to_hf(
                '../../../ceph_data/output/bart-large-AAE-v2-dot-sub/{}/{}'.format(i, file),
                '../../../ceph_data/output/bart-large-AAE-v2-dot-sub/0/best_tfmr',
                '../../../ceph_data/output/bart-large-AAE-v2-dot-sub/{}/best_tfmr2'.format(i)
                )