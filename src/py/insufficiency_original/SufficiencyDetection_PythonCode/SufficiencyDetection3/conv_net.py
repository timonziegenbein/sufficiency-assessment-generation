...
def train_conv_net(datasets,
    ...
    new_val_data = np.random.permutation(new_val_data)
    n_batches = new_val_data.shape[0] / batch_size
    n_val_batches = int(np.round(n_batches))

    val_set = new_val_data[:n_train_batches*batch_size, :]
    val_set_x, val_set_y = shared_dataset((val_set[:, :img_h], val_set[:, -1]))

    print "train: ", len(datasets[0])
    print "dev  : ", len(datasets[1])
    print "test : ", len(datasets[2])
    ...
    test_model_all = theano.function([x, y], [test_error, test_y_pred])
    ...
    while (epoch < n_epochs):
        ...
        if val_perf >= best_val_perf:
            ...
            best_epoch = epoch
            test_loss = test_model_all(test_set_x, test_set_y)[0]
            predictions = test_model_all(test_set_x, test_set_y)[1]
            # print len(predictions_testingtest)
            test_perf = 1 - test_loss
    return {"acc": test_perf, "pred": predictions, "best_epoch": best_epoch}
...
if __name__=="__main__":
    print "loading data...",
    x = cPickle.load(open("data/suff.p", "rb"))
    ...
    datasets = make_idx_data(revs, word_idx_map, max_l=181, k=300, filter_h=5)
    results = train_conv_net(datasets,
                            U,
                            lr_decay=0.95,
                            filter_hs= [1], # [2,3,4],
                            ...
                            batch_size=25,
                            dropout_rate=[0.5])
    print "Perf: " + str(results['acc']) + " at epoch " + str(results['best_epoch'])
