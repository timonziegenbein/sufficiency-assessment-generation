...
def calc_generative_metrics(preds, target) -> Dict:
    ...
    rouge_scores = rouge.compute(predictions=preds, references=target, rouge_types=["rouge1", "rouge2", "rougeLsum"], use_stemmer=True)
    ...
    rougeL = rouge_scores['rougeLsum'][1][-1]*100
...
def run_generate():
    ...
    scores = calc_generative_metrics(output_lns, reference_lns)
